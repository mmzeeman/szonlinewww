...
{% if (id.body or id.blocks) and not is_landscape %}
    <div class="depiction {% if dep.width < 400 or 10*dep.width / dep.height < 18 %}portrait{% else %}landscape{% endif %}">
        {% if dep.width < 400 %}
            <img class="depiction" src="{% image_url dep mediaclass="base-page-main-small" %}" alt="{{ dep.id.title }}" />
        {% else %}
            <img class="depiction" src="{% image_url dep mediaclass="base-page-main" %}" alt="{{ dep.id.title }}" />
        {% endif %}
        {% if dep.id.summary %}
